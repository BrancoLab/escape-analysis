...
def setup(options):
    ...
    # select videos -- currently using the sample video
    dir = os.path.dirname(__file__)
    filename = os.path.join(dir, 'relative', 'path', 'to', 'file', 'you', 'want')
    options.videos = ["C:\\Users\\SWC\\Desktop\\my vid\\wall up.mp4"]
    ...
    options.start_stim_end_frames = [[[0, 3 * 30, 7 * 30], [8 * 30, 11 * 30, 16 * 30], [17 * 30, 18 * 30, 21 * 30]] for x in range(len(options.videos))]
    ...
    options.mouse_silhouette_size = 16 #16 #24 # how big is the mouse in the rendering
