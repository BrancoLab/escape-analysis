...
import os
import warnings
from helper_code.registration_lite import model_arena
...
def visualize_escape(self, video_path, v):
    ...
    # set up model arena
    arena, _ = model_arena((self.height, self.width), self.arena_type[v])
    arena = cv2.cvtColor(arena, cv2.COLOR_GRAY2RGB)  # give it 3 color channels rather than 1
    ...
    for trial, start_stim_end_frame in enumerate(self.start_stim_end_frames[v]):
        ...
        arrived_at_shelter, smoothed_speed, trial_plot, model_mouse_mask_initial, last_darkened_mouse_silhouette, shelter_roi = \
        ...
        while True:
            ...
            if map1: frame = register_frame(frame, self, map1, map2, v)
            else: frame = frame[:,:,0]
            ...
            # continuous shading after stimulus onset
                ...
                if np.sum(shelter_roi * large_ellipse_around_mouse):
        ...
        # save trial images
        escape_image_file_name = os.path.join(os.path.dirname(video_path), 'escape image ' + str(trial) + '.tif')
        imageio.imwrite(escape_image_file_name, visualization_video_frame[:,:,::-1])
        if trial:
            multi_trial_escape_image_file_name = os.path.join(os.path.dirname(video_path), 'escape image ' + str(trial) + ' + previous trials.tif')
...
def initialize_visualization_vars(arena, self):
    ...
    shelter_roi = arena[:,:,0]==150
    last_darkened_mouse_silhouette = 0
    return arrived_at_shelter, smoothed_speed, trial_plot, model_mouse_mask_initial, last_darkened_mouse_silhouette, shelter_roi


def load_fisheye_correction(self):
    # load fisheye mapping if applicable
    if self.registration_data[3]:
        maps = np.load(self.folders['fisheye_map_location']);
        map1 = maps[:, :, 0:2];
        map2 = maps[:, :, 2] * 0
    else:
        map1 = None; map2 = None
...
def set_up_multi_trial_video(self, video_path):
    ...
    multi_trial_visualization_file_name = os.path.join(os.path.dirname(video_path), os.path.basename(video_path)[:-4] + ' multi-trial visualization.mp4')
    fourcc = cv2.VideoWriter_fourcc(*"XVID") # video compression codec
...
def set_up_video_clips(self, trial, video_path):
    # set up the escape clips for saving
    visualization_file_name = os.path.join(os.path.dirname(video_path), os.path.basename(video_path)[:-4] + ' visualization ' + str(trial) + '.mp4')
    video_file_name = os.path.join(os.path.dirname(video_path), os.path.basename(video_path)[:-4] + ' clip ' + str(trial) + '.mp4')
    fourcc = cv2.VideoWriter_fourcc(*"XVID") # video compression codec
...
def get_mouse_silhouette(coordinates, frame_num, model_mouse_mask_initial, scale = 16):
    ...
    neck_angle = coordinates['neck_angle'][frame_num]
    nack_angle = coordinates['nack_angle'][frame_num]
    head_location = tuple(coordinates['head_location'][:,frame_num-1].astype(np.uint16))
    nack_location = tuple(coordinates['nack_location'][:, frame_num].astype(np.uint16))
    front_location = tuple(coordinates['front_location'][:, frame_num].astype(np.uint16))
    ...
    mouse_silhouette = cv2.ellipse(mouse_silhouette, nack_location, (int(scale * .7), int(scale * .35)), 180 - nack_angle, 0, 360, 100, thickness=-1)
    # if turning, down't show the shoulder bit
    if abs(neck_angle - nack_angle) < 40: mouse_silhouette = cv2.ellipse(mouse_silhouette, front_location, (int(scale * .5), int(scale * .33)), 180 - neck_angle, 0, 360, 100, thickness=-1)
    if abs(head_angle - nack_angle) < 40: mouse_silhouette = cv2.ellipse(mouse_silhouette, head_location, (int(scale * .6), int(scale * .3)), 180 - head_angle, 0, 360, 100, thickness=-1)
    if abs(body_angle - shoulder_angle) < 30: mouse_silhouette = cv2.ellipse(mouse_silhouette, shoulder_location, (int(scale), int(scale * .4)), 180 - shoulder_angle, 0, 360, 100, thickness=-1)
    # make a single large ellipse used to determine when do use the flight_color_dark
    large_ellipse_around_mouse = cv2.ellipse(model_mouse_mask_initial.copy(), body_location, (int(scale * 2.5), int(scale * 1.5)), 180 - shoulder_angle, 0, 360, 100,thickness=-1)
...
def register_frame(frame, self, map1, map2, v):
    ...
    x_offset, y_offset = offset[v][0], offset[v][1]
    # make into 2D
