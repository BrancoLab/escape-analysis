...
def model_arena(shape, arena_type = 'circle with shelter'):
    ...
    if arena_type == 'circle with shelter':
        ...
        # initialize model arena *** must be the same dimensions as the video ***
        model_arena = 255 * np.ones((1000, 1000)).astype(np.uint8)
        if model_arena.shape[1]/model_arena.shape[0] != shape[1] / shape[0]: print(colored('Model and video aspect ratio do not agree!!', 'red'))
        # shelter - at value of 150 (this can be used during visualization to stop the video once the mouse reaches the shelter)
        cv2.rectangle(model_arena, (int(500 - 50), int(500 + 408 - 50)), (int(500 + 50), int(500 + 408 + 50)), 150, thickness=-1)
        # arena outline
    ...
    model_arena = cv2.resize(model_arena, shape[::-1]) #, interpolation=cv2.INTER_NEAREST)
    click_points = click_points * [shape[0] / 1000, shape[1] / 1000]

    # show arena
    # cv2.imshow('arena',model_arena)

    return model_arena, click_points
...
def register_arena(background, width, height, fisheye_correction, offset, arena_type):
    ...
    arena, click_points = model_arena((height, width), arena_type = arena_type)
