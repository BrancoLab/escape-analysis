...
def compute_speed_position_angles(processing):
    ...
    processing.coordinates['snout_location'] = np.nanmean(all_body_parts[:, 0:3, :], axis=1)
    processing.coordinates['head_location'] = np.nanmean(all_body_parts[:, 0:6, :], axis=1)
    processing.coordinates['neck_location'] = np.nanmean(all_body_parts[:, 3:6, :], axis=1)
    processing.coordinates['center_location'] = np.nanmean(all_body_parts[:, :, :], axis=1)
    processing.coordinates['nack_location'] = np.nanmean(all_body_parts[:, 3:9, :], axis=1)
    processing.coordinates['front_location'] = np.nanmean(all_body_parts[:, 0:9, :], axis=1)
    processing.coordinates['shoulder_location'] = np.nanmean(all_body_parts[:, 6:9, :], axis=1)
    processing.coordinates['center_body_location'] = np.nanmean(all_body_parts[:, 6:12, :], axis=1)
    processing.coordinates['butty_location'] = np.nanmean(all_body_parts[:, 6:, :], axis=1)
    processing.coordinates['butt_location'] = np.nanmean(all_body_parts[:, 9:, :], axis=1)
    # compute speed
    ...
    locations = ['speed', 'snout_location', 'head_location', 'butt_location', 'neck_location', 'shoulder_location', 'nack_location',
    ...
    # compute angles
    processing.coordinates['body_angle'] = np.angle((processing.coordinates['shoulder_location'][0] - processing.coordinates['butty_location'][0]) + (-processing.coordinates['shoulder_location'][1] + processing.coordinates['butty_location'][1]) * 1j, deg=True)
    processing.coordinates['shoulder_angle'] = np.angle((processing.coordinates['head_location'][0] - processing.coordinates['center_body_location'][0]) + (-processing.coordinates['head_location'][1] + processing.coordinates['center_body_location'][1]) * 1j, deg=True)
    ...
    processing.coordinates['neck_angle'] = np.angle((processing.coordinates['head_location'][0] - processing.coordinates['shoulder_location'][0]) + (-processing.coordinates['head_location'][1] + processing.coordinates['shoulder_location'][1]) * 1j, deg=True)
    processing.coordinates['nack_angle'] = np.angle((processing.coordinates['head_location'][0] - processing.coordinates['nack_location'][0]) + (-processing.coordinates['head_location'][1] + processing.coordinates['nack_location'][1]) * 1j, deg=True)
